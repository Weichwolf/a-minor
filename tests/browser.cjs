...
(async () => {
  ...
  try {
    ...
    await page.selectOption('#language','de');
    await page.goto(base+'/#/');await page.waitForSelector('.tracks');assert.equal(await page.locator('.track').count(),2,'the course keeps two instrument tracks');
    assert.equal(await page.locator('nav a[href="#/theory"]').count(),1,'theory is a main menu item');
    ...
    await page.selectOption('#language','de');
    await page.goto(base+'/#/chapter/cC');await page.waitForSelector('.chapter');
    const appendix=await page.locator('main').innerText();assert.match(appendix,/60000/);assert.match(await page.locator('.crumbs').innerText(),/Anhänge/);
    assert.equal(await page.locator('svg.score').count(),0,'appendices are reference tables without examples');
    await page.setViewportSize({width:390,height:844});await page.goto(base+'/#/chapter/cE');await page.waitForSelector('.chapter');
    assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'appendix tables fit mobile width');
    await page.goto(base+'/#/chapter/c14');await page.waitForSelector('svg.score');
    ...
    await page.selectOption('#fretboard-controls [name=scale]','lydian');
    await page.selectOption('#fretboard-controls [name=show]','degree');
    assert((await page.locator('#fretboard-view .dot text').allTextContents()).includes('#4'));
    await page.selectOption('#language','en');
    assert.equal(await page.locator('#fretboard-controls [name=scale]').inputValue(),'lydian');
    assert(!/undefined|NaN/.test(await page.locator('main').innerText()));
    await page.setViewportSize({width:390,height:844});await page.selectOption('#fretboard-controls [name=frets]','24');
    assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'fretboard scroll stays inside its container');
    await page.screenshot({path:'test-results/fretboard-mobile.png',fullPage:true});
    await page.goto(base+'/#/chapter/c02b');await page.waitForSelector('#c02b-c4 .fb');
    assert.equal(await page.locator('#c02b-c4 .fb [data-midi="60"]').count(),5);
    assert.equal(await page.locator('#c02b-c4 .pk [data-midi="60"]').count(),1);
    assert.equal(await page.locator('#c02b-c4 .illustration').count(),2);
    assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'visible theory maps stay inside mobile page');
    await page.locator('#c02b-c4').screenshot({path:'test-results/theory-c4-mobile.png'});
    await page.goto(base+'/#/chapter/c15');await page.waitForSelector('.rhythm-grid');
    assert.equal(await page.locator('.rhythm-grid').count(),4);
    assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'rhythm grids fit mobile page');
    await page.goto(base+'/#/chapter/c22');await page.waitForSelector('.envelope');
    assert(await page.locator('.visual-flow').count()>0);
    await page.locator('#c22-hold .illustration').screenshot({path:'test-results/theory-envelope-mobile.png'});
    await page.goto(base+'/#/midi');await page.waitForSelector('#connect');
    assert(!/MX49|SR-?18|Nord Lead|Blofeld/i.test(await page.locator('main').innerText()));
    console.log('PASS fretboard explorer, interval spelling, theory maps, rhythm grids and generic MIDI setup');
